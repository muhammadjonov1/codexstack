...
export async function createAnswer(params: CreateAnswerParams) {
  try {
    ...
    if (author.toString() !== questionObject.author.toString()) {
...
export async function deleteAnswer(params: DeleteAnswerParams) {
  try {
    ...
    // Removing reference from the question
    await Question.updateOne(
      { _id: answer.question },
      { $pull: { answers: answerId } }
    );

    // Removing interactions related to the answer
    await Interaction.deleteMany({ answer: answerId });

    // Decrement author's reputation by 5
    await User.findByIdAndUpdate(answer.author, { $inc: { reputation: -10 } });

    // Revalidating the path
    revalidatePath(path);
  } catch (error) {
    console.error("Error deleting answer:", error); // Improved error logging
